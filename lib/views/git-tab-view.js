...
export default class GitTabView extends React.Component {
    ...
    if (this.props.repository.isTooLarge()) {
      ...
      return (
        <div
          ...
          <EtchWrapper ref={c => { this.refCommitViewController = c; }}>
  ...
  rememberFocus(event) {
    ...
    if (!currentFocus && this.refCommitViewController) {
      currentFocus = this.refCommitViewController.getWrappedComponent().rememberFocus(event);
    }
  ...
  setFocus(focus) {
    ...
    if (this.refCommitViewController) {
      if (this.refCommitViewController.getWrappedComponent().setFocus(focus)) {
        return true;
      }
    }
  ...
  advanceFocus(evt) {
    if (!this.refStagingView.getWrappedComponent().activateNextList()) {
      if (this.refCommitViewController.getWrappedComponent().setFocus(GitTabView.focus.EDITOR)) {
        evt.stopPropagation();
      }
    } else {
      evt.stopPropagation();
  ...
  retreatFocus(evt) {
    ...
    const commitViewController = this.refCommitViewController.getWrappedComponent();
